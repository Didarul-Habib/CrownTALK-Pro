...
import { History, Trash2, ChevronRight, Download } from "lucide-react";
  ...
  return (
    <div className={clsx("ct-card", "p-4")}>
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          {onExport ? (
            <button type="button" className="ct-btn ct-btn-xs" onClick={onExport} title="Export history">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <button
          type="button"
          className={clsx("ct-btn ct-btn-xs", modeFilter === "all" ? "ct-btn-primary" : "")}
        ...
        <button
          type="button"
          className={clsx("ct-btn ct-btn-xs", modeFilter === "urls" ? "ct-btn-primary" : "")}
          onClick={() => setModeFilter("urls")}
        >
          URLs
        </button>
        <button
          type="button"
          className={clsx("ct-btn ct-btn-xs", modeFilter === "source" ? "ct-btn-primary" : "")}
          onClick={() => setModeFilter("source")}
        >
          Source
        </button>
      </div>
      ...
      {!filteredRuns.length ? (
        ...
        <div className="mt-3">
          <List height={listHeight} itemCount={filteredRuns.length} itemSize={rowHeight} width={"100%"} overscanCount={3}>
            {({ index, style }) => {
                  ...
                  <div className={clsx("ct-card-surface", "p-3", "flex items-center justify-between gap-3")}>
                    <div className="min-w-0">
                      ...
                      <div className="mt-0.5 text-sm font-semibold tracking-tight truncate">
                        {r.mode === "source" ? "Source" : "URLs"} • {r.results.length} • {r.okCount} ok • {r.failedCount} failed
                      </div>
                    </div>

                    <div className="flex items-center gap-2 shrink-0">
                      <button type="button" className="ct-btn ct-btn-xs" onClick={() => onLoad(r.id)} title="Load this run">
